...
class ZohoService {
    ...
    static tokenExpiresAt = 0; // Unix timestamp in ms
    ...
    static async searchContacts(searchText) {
        try {
    ...
    static async searchVendors(searchText) {
        try {
            const accessToken = await this.getAccessToken();
            const orgId = process.env.ZOHO_ORG_ID;
            const apiUrl = process.env.ZOHO_BOOKS_API_URL || 'https://www.zohoapis.com/books/v3';
            ...
            const companyData = companyRes.ok ? await companyRes.json() : { contacts: [] };
    ...
    static async searchItems(searchText) {
        try {
            const accessToken = await this.getAccessToken();
            const orgId = process.env.ZOHO_ORG_ID;
            const apiUrl = process.env.ZOHO_BOOKS_API_URL || 'https://www.zohoapis.com/books/v3';
