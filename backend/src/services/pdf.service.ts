...
export class PdfService {
  static async generateQuotationPdf(quotation: any, job: any, items: any[]): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (gstNo && gstNo.length >= 2) {
        ...
        if (addr.includes('kerala')) placeOfSupply = 'Kerala (32)';
        else if (addr.includes('tamil nadu') || addr.includes('tamilnadu')) placeOfSupply = 'Tamil Nadu (33)';
        else if (addr.includes('maharashtra')) placeOfSupply = 'Maharashtra (27)';
        else if (addr.includes('delhi')) placeOfSupply = 'Delhi (07)';
        else if (addr.includes('karnataka')) placeOfSupply = 'Karnataka (29)';
      }
  ...
  static async generateLowStockPdf(lowParts: any[]): Promise<string> {
  ...
  static async generateServiceReportPdf(report: any, job: any, tests: any, parts: any[]): Promise<string> {
  ...
  static async generateQcAssessmentPdf(assessment: any, job: any): Promise<string> {
  ...
  static async generateCustomPoPdf(poData: {
    ...
    items: Array<{ name: string; description?: string; qty: number; rate: number }>;
    notes?: string;
  }): Promise<string> {
