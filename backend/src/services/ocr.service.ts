...
export class OcrService {
  static async scanNameplate(filePath: string): Promise<OcrResult> {
  ...
  private static parseText(text: string): OcrResult {
    const rawText = text;
    const normalized = text.toUpperCase();

    // 1. Extract Brand
    let brand = 'Other';
    if (normalized.includes('IPG')) brand = 'IPG';
    else if (normalized.includes('RAYCUS')) brand = 'Raycus';
    else if (normalized.includes('MAXPHOTONICS') || normalized.includes('MAX')) brand = 'Maxphotonics';
    else if (normalized.includes('JPT')) brand = 'JPT';
    else if (normalized.includes('NLIGHT') || normalized.includes('N-LIGHT')) brand = 'nLIGHT';
    else if (normalized.includes('BWT')) brand = 'BWT';
